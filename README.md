...
│   │   └── html_renderer.py       # HTML报表渲染器（严格复刻XLSX样式）
│   ├── utils/                     # 工具模块
│   │   └── logger.py              # 日志配置
│   ├── gui/                       # GUI桌面应用模块
│   │   ├── app.py                 # 主窗口（Tkinter）
...
支持两种输出格式（通过 `settings.yaml` 中的 `output.format` 配置）：
...
- **取数**：完全基于 Code 表动态查询 5101/5102/5501/5502/5503 科目体系，不再硬编码 6001/6401/6601
- **合计**：汇总行合计、利润/利润率、所有明细行合计列均已自动计算
- **银行余额**：年度合计列已排除（时点数不计年度合计）
- **空白行**：背景色固定为纯白色，与 Excel 表现一致
- **HTML 输出**：已完成 HTML 报表渲染器，支持 XLSX + HTML 双格式输出
- **已知限制**：若账套在 Code 表中无 5101 科目或 5101 无任何子科目，且期间损益结转凭证中无数据，则该账套收入行将显示为空
